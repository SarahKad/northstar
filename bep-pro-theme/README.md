...
A bold, dark-first design token theme for BEP Pro products. Anchored to **BEP Blue** (`#1A56DB`).
...
| File | Purpose |
|---|---|
| `bep-pro.css` | All design tokens, primitives, semantic tokens, chart series, badge series |
...
Copy the contents of `tailwind-theme-snippet.css` into your `@theme inline { … }` block in `globals.css`. This registers the CSS variables as Tailwind utility classes (`bg-primary`, `text-muted-foreground`, `bg-bep-badge-1`, etc.).

### 3. Activate the theme

Set `data-theme="bep-pro"` on your root `<html>` element (or any container element to scope it):

```html
<html data-theme="bep-pro">
```

To activate **light mode** within BEP Pro, add both attributes:

```html
<html data-theme="bep-pro" class="light">
```

BEP Pro defaults to **dark mode**. Light mode is the secondary variant.
...
| `--brand` | `#3D82F0` | `#1A56DB` | BEP Blue, lightened for dark bg |
...
| Token | Colour | Hex |
|---|---|---|
| `--chart-1` | BEP Light Blue | `#60A5FA` |
| `--chart-2` | BEP Orange | `#FB923C` |
| `--chart-3` | BEP Purple | `#AC94FA` |
| `--chart-4` | BEP Peach | `#FDBA8C` |
| `--chart-5` | BEP Red | `#EF4444` |
| `--chart-6` | BEP Dark Red | `#7F1D1D` |
| `--chart-7` | BEP Lavender | `#DDD6FE` |
...
These tokens are only defined when `[data-theme="bep-pro"]` is active. In any other theme context they are undefined/transparent.

| Token | Colour | Hex |
|---|---|---|
| `--bep-badge-1` | BEP Light Blue | `#60A5FA` |
| `--bep-badge-2` | BEP Purple | `#AC94FA` |
| `--bep-badge-3` | BEP Lavender | `#DDD6FE` |
| `--bep-badge-4` | BEP Orange | `#FB923C` |
| `--bep-badge-5` | BEP Peach | `#FDBA8C` |
| `--bep-badge-6` | BEP Dark Red | `#7F1D1D` |
| `--bep-badge-7` | BEP Red | `#EF4444` |

### Gradient token

`--bep-accent-gradient`, a linear gradient from orange → red → BEP blue. Badges and charts only.

```css
background: var(--bep-accent-gradient);
...
BEP Pro does not ship custom fonts. Recommended stack:
...
- **Display / headings (optional):** Any high-contrast serif or system font. BEP Pro's dark-first palette works well with neutral weight display type.
...
2. **Accent / badge palette is restricted**, `--chart-*` for data viz only, `--bep-badge-*` for badges/tags only.
3. **Dark is the default**, BEP Pro dark should be treated as the primary state, not an override.
4. **`--radius` is fixed at `4px`**, do not override per-component without explicit design approval.
5. **`--bep-badge-*` tokens are theme-scoped**, they are undefined outside `[data-theme="bep-pro"]`. Never reference them in shared/cross-theme components.
